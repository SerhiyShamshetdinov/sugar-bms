...
class MonadicBooleanCollectionFeaturesTest[M[_]](implicit BCM: BooleanCollectionMonad[M, NoContext]) extends AnyFlatSpec with Matchers {
  ...
  behavior of BCM.name

  def mPure[A](a: A*): M[A] = BCM.pure(a: _*)

  val mFalse: M[Boolean] = mPure(false)
  val mTrue:  M[Boolean] = mPure(true)
  val mUnit:  M[Unit] =    mPure({})

  val mFalseFalse: M[Boolean] = mPure(false, false)
  val mFalseTrue: M[Boolean]  = mPure(false, true)
  val mTrueFalse: M[Boolean]  = mPure(true, false)
  val mTrueTrue: M[Boolean]   = mPure(true, true)

  def mEmpty[A]: M[A] = BCM.empty[A]

  val mBooleanEmpty: M[Boolean] = mEmpty[Boolean]

  it should "have type constants" in {

    BCM.False shouldBe mFalse
    BCM.True  shouldBe mTrue
    BCM.Unit  shouldBe mUnit

// >>> other ways to get constants for a Boolean Collection are the following (may not be generalized here)
...
//    List.Unit
  ...
  /// >>> the following expressions test is copy-paste for Monadic Boolean Collections and Monadic Array
  ...
  it should "have logical operators extensions" in {
    ...
    false && mFalseTrue  shouldBe mFalseFalse // lazy && does not work for collections to always have precise result length
    true  || mFalseFalse shouldBe mTrueTrue  // lazy || does not work for collections to always have precise result length
    true  &  mFalseTrue  shouldBe mFalseTrue
    false ^  mTrueFalse  shouldBe mTrueFalse
    false |  mFalseFalse shouldBe mFalseFalse

    mFalseTrue  && true  shouldBe mFalseTrue // here lazy && works since the "length" of pure Boolean is known
    mTrueFalse  || false shouldBe mTrueFalse  // here lazy || works since the "length" of pure Boolean is known
    mFalseTrue  &  true  shouldBe mFalseTrue
    mTrueFalse  ^  false shouldBe mTrueFalse
    mFalseFalse |  false shouldBe mFalseFalse

    mTrueFalse  && mTrue   shouldBe mTrueFalse
    mFalseFalse || mFalse  shouldBe mFalseFalse
    mFalseTrue  &  mFalse  shouldBe mFalseFalse
    mFalseTrue  ^  mTrue   shouldBe mTrueFalse
    mFalseFalse |  mFalse  shouldBe mFalseFalse

    mTrueFalse  && mTrueTrue   shouldBe mPure(true, true, false, false)
    mFalseFalse || mFalseTrue  shouldBe mPure(false, true, false, true)
    mFalseTrue  &  mFalseTrue  shouldBe mPure(false, false, false, true)
    mFalseTrue  ^  mTrueFalse  shouldBe mPure(true, false, false, true)
    mFalseFalse |  mFalseFalse shouldBe mPure(false, false, false, false)
    ...
    true && mFalseTrue || mTrue ^ mTrueTrue & mFalseFalse | false shouldBe mPure(true, true, true, true, true, true, true, true)
  }
