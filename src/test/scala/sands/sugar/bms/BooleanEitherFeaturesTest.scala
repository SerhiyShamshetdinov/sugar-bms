...
class BooleanEitherFeaturesTest extends AnyFlatSpec with Matchers {

  behavior of "Boolean Either"

  def mPure[A](a: A): Right[String, A] = Right[String, A](a)

  val mFalse: Right[String, Boolean] = mPure(false)
  val mTrue:  Right[String, Boolean] = mPure(true)
  val mUnit:  Right[String, Unit] =    mPure({})

  def mEmpty[A]: Left[String, A] = Left[String, A]("No Boolean")

  val mBooleanEmpty: Left[String, Boolean] = mEmpty[Boolean]

  it should "have type constants" in {

    Either.False[String] shouldBe mFalse
    Either.True[String]  shouldBe mTrue
    Either.Unit[String]  shouldBe mUnit
  }
  ...
  it should "have flat monadic 'if' extensions and standalone methods" in {
    ...
    mFalse.flatMapIf(mPure(5)) shouldBe mUnit
    ...
    flatMapIf(mFalse)(mPure(5)) shouldBe mUnit
    ...
    mFalse.fmIf(mPure(5)) shouldBe mUnit
    ...
    fmIf(mFalse)(mPure(5)) shouldBe mUnit
