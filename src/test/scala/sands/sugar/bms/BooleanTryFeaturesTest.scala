...
class BooleanTryFeaturesTest extends AnyFlatSpec with Matchers {

  behavior of "Boolean Try"

  def mPure[A](a: A): Success[A] = Success[A](a)

  val mFalse: Success[Boolean] = mPure(false)
  val mTrue:  Success[Boolean] = mPure(true)
  val mUnit:  Success[Unit] =    mPure({})

  val noBooleanException = new Exception("No Boolean")
  def mEmpty[A]: Failure[A] = Failure[A](noBooleanException)

  val mBooleanEmpty: Failure[Boolean] = mEmpty[Boolean]

  it should "have type constants" in {

    Try.False shouldBe mFalse
    Try.True  shouldBe mTrue
    Try.Unit  shouldBe mUnit
  }
