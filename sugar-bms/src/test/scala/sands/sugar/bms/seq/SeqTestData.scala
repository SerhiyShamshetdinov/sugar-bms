...
object SeqTestData extends BmsCollectionTestData[Seq] {
  val TypeImports = Seq()
  val MonadName = "Seq"
  val AsMonadImports = Seq("import sands.sugar.bms.typeclass.BooleanMonad.CollectionsAsMonad.seqBooleanMonad")
  val TestDataImports = Seq("import sands.sugar.bms.seq.SeqTestData.{Empty, lift}")

  def lift(b: Boolean): Seq[Boolean] = if(b) PureTrue else PureFalse
  def lift(bl: Seq[Boolean]): Seq[Boolean] = bl

  def pure[T: ClassTag](v: T*): Seq[T] = v.toSeq
  val sPure = "Seq"

  val PureFalse: Seq[Boolean] = pure(false)
  val sPureFalse = s"$sPure(false)"
  val PureTrue: Seq[Boolean] = pure(true)
  val sPureTrue = s"$sPure(true)"
  val PureUnit: Seq[Unit] = pure(())
  val Empty: Seq[Nothing] = Seq()
  ...
  val BooleanEmpty: Seq[Boolean] = Seq[Boolean]()
  val sBooleanEmpty = "Seq[Boolean]()"

  val tryGetBoolean: Try[Seq[Boolean]] => Try[Boolean] = _.flatMap(mc => if (mc.length != 1) Failure(NoValueException) else Success(mc.head))
