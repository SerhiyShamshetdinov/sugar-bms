...
object VectorTestData extends BmsCollectionTestData[Vector] {
  val TypeImports = Seq()
  val MonadName = "Vector"
  val AsMonadImports = Seq("import sands.sugar.bms.typeclass.BooleanMonad.CollectionsAsMonad.vectorBooleanMonad")
  val TestDataImports = Seq("import sands.sugar.bms.vector.VectorTestData.{Empty, lift}")

  def lift(b: Boolean): Vector[Boolean] = if(b) PureTrue else PureFalse
  def lift(bl: Vector[Boolean]): Vector[Boolean] = bl

  def pure[T: ClassTag](v: T*): Vector[T] = v.toVector
  val sPure = "Vector"

  val PureFalse: Vector[Boolean] = pure(false)
  val sPureFalse = s"$sPure(false)"
  val PureTrue: Vector[Boolean] = pure(true)
  val sPureTrue = s"$sPure(true)"
  val PureUnit: Vector[Unit] = pure(())
  ...
  val BooleanEmpty: Vector[Boolean] = Vector[Boolean]()
  val sBooleanEmpty = "Vector[Boolean]()"

  val tryGetBoolean: Try[Vector[Boolean]] => Try[Boolean] = _.flatMap(mc => if (mc.length != 1) Failure(NoValueException) else Success(mc.head))
