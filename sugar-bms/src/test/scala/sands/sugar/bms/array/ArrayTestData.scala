...
object ArrayTestData extends BmsCollectionTestData[Array] {
  val TypeImports = Seq()
  val MonadName = "Array"
  val AsMonadImports = Seq("import sands.sugar.bms.typeclass.functions.BooleanFunctions.ArrayAsMonad.arrayMonadicBooleanFunctions")
  val TestDataImports = Seq("import sands.sugar.bms.array.ArrayTestData.{AnyRefEmpty => Empty, lift}")

  def lift(b: Boolean): Array[Boolean] = if(b) PureTrue else PureFalse
  def lift(bl: Array[Boolean]): Array[Boolean] = bl

  def pure[T: ClassTag](v: T*): Array[T] = v.toArray
  val sPure = "Array"

  val PureFalse: Array[Boolean] = pure(false)
  val sPureFalse = s"$sPure(false)"
  val PureTrue: Array[Boolean] = pure(true)
  val sPureTrue = s"$sPure(true)"
  val PureUnit: Array[Unit] = pure(())

  def Empty: Array[Nothing] = null // not used: AnyRefEmpty is renamed to Empty by TestDataImports import
  val AnyRefEmpty: Array[AnyRef] = Array[AnyRef]()
  ...
  val BooleanEmpty: Array[Boolean] = Array[Boolean]()
  val sBooleanEmpty = "Array[Boolean]()"

  val tryGetBoolean: Try[Array[Boolean]] => Try[Boolean] = _.flatMap(mc => if (mc.length != 1) Failure(NoValueException) else Success(mc.head))
  ...
  // in 2.13 6 List argument variants (1280 runBmsCase in 1 test) fail in toolbox test quaternaryBooleanExpressionExpectedMonadicOpsPattern with error:
