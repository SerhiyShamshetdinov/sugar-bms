...
object ListTestData extends BmsCollectionTestData[List] {
  val TypeImports = Seq()
  val MonadName = "List"
  val AsMonadImports = Seq("import sands.sugar.bms.typeclass.BooleanMonad.CollectionsAsMonad.listBooleanMonad")
  val TestDataImports = Seq("import sands.sugar.bms.list.ListTestData.{Empty, lift}")

  def lift(b: Boolean): List[Boolean] = if(b) PureTrue else PureFalse
  def lift(bl: List[Boolean]): List[Boolean] = bl

  def pure[T: ClassTag](v: T*): List[T] = v.toList
  val sPure = "List"

  val PureFalse: List[Boolean] = pure(false)
  val sPureFalse = s"$sPure(false)"
  val PureTrue: List[Boolean] = pure(true)
  val sPureTrue = s"$sPure(true)"
  val PureUnit: List[Unit] = pure(())
  ...
  val BooleanEmpty: List[Boolean] = List[Boolean]()
  val sBooleanEmpty = "List[Boolean]()"

  val tryGetBoolean: Try[List[Boolean]] => Try[Boolean] = _.flatMap(mc => if (mc.length != 1) Failure(NoValueException) else Success(mc.head))
