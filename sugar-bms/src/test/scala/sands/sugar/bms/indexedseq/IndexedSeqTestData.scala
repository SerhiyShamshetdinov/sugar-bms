...
object IndexedSeqTestData extends BmsCollectionTestData[IndexedSeq] {
  val TypeImports = Seq()
  val MonadName = "IndexedSeq"
  val AsMonadImports = Seq("import sands.sugar.bms.typeclass.BooleanMonad.CollectionsAsMonad.indexedSeqBooleanMonad")
  val TestDataImports = Seq("import sands.sugar.bms.indexedseq.IndexedSeqTestData.{Empty, lift}")

  def lift(b: Boolean): IndexedSeq[Boolean] = if(b) PureTrue else PureFalse
  def lift(bl: IndexedSeq[Boolean]): IndexedSeq[Boolean] = bl

  def pure[T: ClassTag](v: T*): IndexedSeq[T] = v.toIndexedSeq
  val sPure = "IndexedSeq"

  val PureFalse: IndexedSeq[Boolean] = pure(false)
  val sPureFalse = s"$sPure(false)"
  val PureTrue: IndexedSeq[Boolean] = pure(true)
  val sPureTrue = s"$sPure(true)"
  val PureUnit: IndexedSeq[Unit] = pure(())
  ...
  val BooleanEmpty: IndexedSeq[Boolean] = IndexedSeq[Boolean]()
  val sBooleanEmpty = "IndexedSeq[Boolean]()"

  val tryGetBoolean: Try[IndexedSeq[Boolean]] => Try[Boolean] = _.flatMap(mc => if (mc.length != 1) Failure(NoValueException) else Success(mc.head))
